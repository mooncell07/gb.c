...
static inline uint8_t getReg8(Bus *bus, R8Type r) {
    if (r == aHL) {
        return readByte(bus, getReg16(HL), true, true);
    }
    return r8[r];
}

static inline void setReg8(Bus *bus, R8Type r, uint8_t n) {
    if (r == aHL) {
        writeByte(bus, getReg16(HL), n, true);
    } else {
        r8[r] = n;
    }
}

...
uint16_t getTileRow(Fetcher *self) {
    if (!self->isWindowVisible) {
        return ((uint16_t)(self->staticLine % 8) * 2);
    }
...
uint16_t internalY(Fetcher *self) {
    if (!self->isWindowVisible) {
        return ((uint16_t)(((self->mmio->LY + self->mmio->SCY) & 0xFF) / 8) *
                32);
    }
...
void spriteTick(Fetcher *self) {
    ...
    case 0:
        ...
        if (self->currSprite.height == 16)
            self->currSprite.tileNum &= 0xFE;
    ...
    case 2:
        // Horizontal flip.
        int counter = 0;
        if (self->currSprite.x < 8)
            counter = 8 - self->currSprite.x;
        if (BT(self->currSprite.flags, 5))
            flipX(self);
        ...
        for (; counter < 8; counter++) {
            ...
            if (residualAmount >= (counter + 1)) {
                Pixel residualPixel =
                    getEntryAt(self->sFIFO, &self->sFIFO, counter);

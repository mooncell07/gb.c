...
double FRAME_DURATION = 1.0 / 59.7;
...
PPU *createPPU(MMIO *mmio) {
  ...
  if (ppu != NULL) {
    ...
    ppu->windowInit = true;
    ppu->mmio = mmio;
    ppu->startTime = SDL_GetPerformanceCounter();
    ppu->freq = SDL_GetPerformanceFrequency();
  }
  return ppu;
}
...
void sortSprites(PPU *self) {
  ...
  while (i < FIFOLen(&self->fetcher->sprites.base)) {
    ...
    while (
        j > 0 &&
        getEntryAt(self->fetcher->sprites, &self->fetcher->sprites, j - 1).x >
            next.x) {
      setEntryAt(
          self->fetcher->sprites, j,
          getEntryAt(self->fetcher->sprites, &self->fetcher->sprites, j - 1));
      j = j - 1;
    }
...
void searchSprites(PPU *self) {
  for (int i = 0; i < 40; i++) {
    if (FIFOLen(&self->fetcher->sprites.base) == 10) { break; }
    ...
    if (getLCDC(self->mmio, OBJSIZE)) { spriteHeight = 16; }

    if (((self->mmio->LY + 16) >= spriteY) &&
        ((self->mmio->LY + 16) < (spriteY + spriteHeight)) && (spriteX > 0)) {
...
void isWindowEnabled(PPU *self) {
  if (self->mmio->LY == self->mmio->WY) { self->windowTrigger = true; }
...
void sendStatInt(PPU *self) {
  ...
  if (!oldState && self->intLineUp) { sendIntReq(self->mmio, INTSTAT); }
}
...
void handleCoincidence(PPU *self, bool quirk) {
  ...
  if (qLY == self->mmio->LYC) {
    ...
    checkStatInt(self, 6) ? sendStatInt(self) : clearBit(self->mmio->STAT, 2);
  }
}

void nextLine(PPU *self) {
  self->mmio->LY++;
  if (self->incrWLY) self->mmio->WLY++;
    ...
    case PIXELTRANSFER:
      if (self->fetcher->tickingSprite) {
        ...
        if (FIFOLen(&self->fetcher->sprites.base) != 0) {
          Sprite spr =
              getEntryAt(self->fetcher->sprites, &self->fetcher->sprites, 0);
      ...
      if (self->fetcher->isWindowVisible) {
        ...
        if (self->windowInit) {
          ...
          self->dropPixels = 0;
          if (self->mmio->WX < 7) { self->dropPixels = 7 - self->mmio->WX; }
          return;
        }
      }
      if (!self->fetcher->tickingSprite) { tileTick(self->fetcher); }
      if ((FIFOLen(&self->fetcher->tFIFO.base) > 0) &&
          !self->fetcher->tickingSprite) {
        Pixel tilePixel = popEntry(self->fetcher->tFIFO, &self->fetcher->tFIFO);
        ...
        if (FIFOLen(&self->fetcher->sFIFO.base) > 0) {
          ...
          uint8_t spritePixelColorIndex = getColorIndex(
              self->fetcher, spritePixel.colorCode, true, spritePixel.palette1);
          ...
          if ((spritePixel.colorCode == 0) ||
            ...
            finalColor =
                getLCDC(self->mmio, OBJEN) ? spritePixelColor : tilePixelColor;
          }
        } else
          finalColor = tilePixelColor;
    ...
    case HBLANK:
      if (self->dots == 456) {
        ...
        if (self->mmio->LY > 143) {
          ...
          double deltaTime = (double)(currTime - self->startTime) / self->freq;
          while (deltaTime < FRAME_DURATION) {
            deltaTime =
                (double)(SDL_GetPerformanceCounter() - self->startTime) /
                    self->freq +
                0.0001;
          }
          // double fps = 1.0 / deltaTime;
          // printf("fps: %0.2f\n", fps);
          self->startTime = SDL_GetPerformanceCounter();

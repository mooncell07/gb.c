...
uint8_t _mbc1Read(Mapper *mapper, uint16_t address) {
    if (address <= 0x3FFF) {
        ...
        uint16_t highBankNum =
            mapper->mbc1.romBank & (mapper->romBankCount - 1);
        ...
        if (mapper->extRamFlag) {
            if ((mapper->ramSizeCode > 0x02) && mapper->mbc1.modeFlag) {
                return mapper->extRam[0x2000 * mapper->mbc1.ramBank +
                                      (address - 0xA000)];
            } else {
...
void _mbc1Write(Mapper *mapper, uint16_t address, uint8_t data) {
    if (address <= 0x1FFF) {
        ...
        if ((mapper->mbc1.romBank == 0) &&
            (maskedData < mapper->romBankCount)) {
        ...
        if (mapper->extRamFlag) {
            if ((mapper->ramSizeCode > 0x02) && mapper->mbc1.modeFlag) {
                mapper->extRam[0x2000 * mapper->mbc1.ramBank +
                               (address - 0xA000)] = data;
            } else {
